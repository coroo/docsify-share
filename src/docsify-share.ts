export const install = (hook: any, vm: any) => {
    let facebook = vm.config.share.facebook != undefined ? '<a href="https://www.facebook.com/sharer.php?u='+ (vm.config.share.facebook.url ?? window.location.href) +'" target="_blank" class="fab indigo" tooltip="Facebook"><i class="fa fa-facebook animated"></i></a>' : '';
    ...
    let telegram = vm.config.share.telegram != undefined ? '<a href="https://telegram.me/share/url?url=&'+ (vm.config.share.telegram.url ?? window.location.href) +'text='+ (vm.config.share.telegram.title ?? document.title) +'" target="_blank" class="fab black" tooltip="Telegram"><i class="fa fa-telegram animated"></i></a>' : '';
